...
module servo_motor_door(
        input clk, reset_p,
        input open_flag,
        output servo_pwm_door);
        
        wire open_nedge, open_pedge;
        edge_detector_n flame_edge(.clk(clk), .reset_p(reset_p), .cp(open_flag), .n_edge(open_nedge), .p_edge(open_pedge));
        
        reg [3:0] duty;
        always @(posedge clk or posedge reset_p)begin
                if(reset_p) begin duty = 3; end  // 초기값 설정 
                else if(!open_flag) begin duty = 3; end   // 0도
                else if(open_pedge) begin duty = 8;  end  // 90도 
        end 
        
        pwm_Nstep_freq #(.duty_step(100), .pwm_freq(50))
                                      pwm_motor(.clk(clk), .reset_p(reset_p), .duty(duty), .pwm(servo_pwm_door));
